 ...
 * @author ZZZank
 */
public class ProvideAE2OpenAction implements OpenActionProvider.RequireMod {
    @Override
    ...
    public void register(OpenActionRegistry registry) {
        registry.registerIfPresent(id("wireless_terminal"), false);
        registry.registerIfPresent(id("wireless_crafting_terminal"), false);
        ...
        for (var capacity : List.of(1, 4, 16, 64, 256)) {
            registry.registerIfPresent(id("portable_item_cell_" + capacity + "k"), false);
            registry.registerIfPresent(id("portable_fluid_cell_" + capacity + "k"), false);
        }
    }
}
